...
func TestGatewayService_AnthropicAPIKeyPassthrough_BuildRequestWritesDebugSnapshot(t *testing.T) {
	...
	req, _, err := svc.buildUpstreamRequestAnthropicAPIKeyPassthrough(context.Background(), c, account, body, "upstream-secret-key")
	require.NoError(t, err)
...
func TestGatewayService_BuildCountTokensRequest_WritesDebugSnapshot(t *testing.T) {
	...
	req, _, err := svc.buildCountTokensRequest(context.Background(), c, account, body, "upstream-secret-key", "apikey", "claude-3-5-sonnet-latest", false)
	require.NoError(t, err)
	require.NotNil(t, req)
